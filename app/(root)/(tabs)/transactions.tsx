...
export default function Transactions() {
    ...
    const [activeFilters, setActiveFilters] = useState<(typeof FILTERS)[number]>("All")
    const [activeAccountId, setActiveAccountId] = useState<string | null>(null)
    const [search, setSearch] = useState("")
    const [exporting, setExporting] = useState(false)

    const typeFilter: TransactionType | null =
        (activeFilters === "Income") ? "INCOME" : (activeFilters === "Expense") ? "EXPENSE" : null
    ...
    const dailyIncomeExpense = useMemo(() => {
        ...
        return days.flatMap(({ key, label }) => {
            const income = transactions
                .filter((tx) =>
                    tx.type === "INCOME" && dayKey(new Date(tx.date)) === key
                )
                .reduce((sum, tx) => sum + tx.amount, 0)

            const expense = transactions
                .filter((tx) =>
                    tx.type === "EXPENSE" && dayKey(new Date(tx.date)) === key
                )
                .reduce((sum, tx) => sum + tx.amount, 0)
        ...
        <SafeAreaView className='flex-1 bg-brand-body' edges={["top"]}>
            <View className='px-5 pt-3 pb-2'>
                ...
                <View className='flex-row items-center gap-2 bg-white rounded-xl border border-[#E8E6DF] px-3.5 py-2.5 mb-2.5'>
                    <Feather name='search' size={25} color={"#8A8D96"} />
                    <TextInput
                        value={search}
                ...
                <View className='flex-row gap-2 mb-2.5'>
                    {FILTERS.map((filter) => (
                        <TouchableOpacity
                            ...
                            className={`px-3.5 py-1.5 rounded-full border ${activeFilters === filter
                                ? "bg-brand-bg border-brand-bg"
                                : "bg-white border-[#E8E6DF]"
                                }`}
                        >
                            <Text
                                className={`text-xs ${activeFilters === filter ? "text-white" : "text-brand-text-secondary"}`}
                            >
                                {filter}
                            </Text>
                        </TouchableOpacity>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    <View className='flex-row gap-2'>
                        ...
                        >
                            <Text
                                className={`text-xs ${activeAccountId === null ? "text-white" : "text-brand-text-secondary"}`}
                            >
                                All Accounts
                            </Text>
                        </TouchableOpacity>
                        ...
                        {accounts.map((account) => (
                            ...
                            >
                                <Text
                                    className={`text-xs ${activeAccountId === account.id ? "text-white" : "text-brand-text-secondary"}`}
                                >
                                    {account.name}
                                </Text>
                            </TouchableOpacity>
